...
export class CaseListComponent {
  ...
  private readonly assignedTo$ = toObservable(this.currentUser).pipe(
    map((user) => (this.scope === 'mine' ? (user?.uid ?? undefined) : undefined)),
  );
  ...
  private readonly response = toSignal(
    combineLatest([this.page$, this.status$, this.search$, this.assignedTo$]).pipe(
      tap(() => this.loadingSignal.set(true)),
      switchMap(([page, status, search, assignedTo]) =>
        this.caseService
          .list({
            page,
            pageSize: PAGE_SIZE,
  ...
  readonly hasActiveFilters = computed(() => this.search().trim().length > 0 || this.statusFilter() !== 'all');
